...
export const FOLDER_CLASS = {
	...
	sessionRow: "dsh-buddy-folder-session-row",
	session: "dsh-buddy-folder-session",
	more: "dsh-buddy-folder-more",
...
export const FOLDER_CSS = [
	...
	`.${c.entry}:focus-visible,.${c.toggle}:focus-visible,.${c.session}:focus-visible,.${c.more}:focus-visible{outline:2px solid var(--dsw-alias-label-primary);outline-offset:-2px}`,
	...
	`.${c.sessionRow}{box-sizing:border-box;width:100%;height:32px;flex:none;display:flex;align-items:center;border-radius:8px}`,
	`.${c.sessionRow}:hover,.${c.sessionRow}.${c.selected}{background:var(--dsw-alias-interactive-bg-hover)}`,
	`.${c.session}{box-sizing:border-box;cursor:pointer;flex:1;min-width:0;height:100%;padding:0 4px 0 8px;display:flex;align-items:center;gap:6px;border:none;border-radius:8px;background:0 0;color:var(--dsw-alias-label-primary);font-family:inherit;text-align:left}`,
	`.${c.more}{flex:none;margin-right:4px;color:var(--dsw-alias-label-secondary)}`,

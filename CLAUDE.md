...
- **The `buddy` preset is a plugin artifact, not a user file.** `syncPreset` (`src/store/preset.ts`) writes the shipped template into `~/.dsh/.agent-presets/buddy/` and rewrites it on upgrade, keyed on the `.dsh-buddy-generated` marker it puts there: a marked install is synchronized file by file, an unmarked directory is someone else's `buddy` preset and is left alone entirely, and a marked file whose bytes do not hash to the baseline the marker recorded is taken for a hand-edit and copied to `<file>.bak` before the overwrite. Editing `assets/preset/agent.cordis.yml` therefore does reach an installed preset. To customize the wiring, copy the preset to a new id — the GUI's "copy preset", `agentPresets.copy` — and edit that copy, never this generated one.
